...
ansible-inventory -i inventory.yml --host rhel-node-01
...
managed_users:
  - name: opsuser
    comment: Operations managed account
    groups:
      - wheel
...
```yaml
---
- name: Configure a RHEL server baseline
  hosts: rhel_managed
  become: true
  gather_facts: true

  pre_tasks:
    - name: Verify the target is supported by this baseline playbook
      ansible.builtin.assert:
        that:
          - ansible_facts.os_family == 'RedHat'
          - ansible_facts.distribution_major_version == '9'
        fail_msg: This example is limited to a reviewed RHEL-compatible version 9 host.
...
ansible-playbook -i inventory.yml linux-baseline.yml --check --diff --limit rhel-node-01
ansible-playbook -i inventory.yml linux-baseline.yml --diff --limit rhel-node-01
ansible-playbook -i inventory.yml linux-baseline.yml --check --diff --limit rhel-node-01
```

最後の check で不要な `changed` が出ないかを確認します。掲載出力は形式を示す例であり、実環境の実測結果として扱いません。
...
この例は control node の `/tmp/ansible-openshift-manifests` へ YAML を配置し、指定された kubeconfig で `oc apply` を呼びます。**実行済みの表明ではなく、検証環境用の具体例です。**
...
具体例では固定した版の HTTP テストイメージを使います。OpenShift の restricted SCC が Project 固有の非 0 UID を割り当てる前提で、Manifest に固定 UID は指定していません。商用では承認済み Registry の immutable digest へ固定し、mirror、署名、脆弱性、EOL、対象 SCC との互換性を確認します。外部 Registry に接続できない環境では内部 mirror へ書き換えます。
  ...
  pre_tasks:
    - name: Require an explicit kubeconfig path
      ansible.builtin.assert:
        ...
        fail_msg: Pass kubeconfig_path and the approved expected_api_server for the target cluster.
...
oc --kubeconfig /secure/path/target.kubeconfig apply --dry-run=server -f files/demo-app.yml
oc --kubeconfig /secure/path/target.kubeconfig diff -f files/demo-app.yml
ansible-playbook openshift-apply.yml -e kubeconfig_path=/secure/path/target.kubeconfig -e expected_api_server=https://api.cluster.example.com:6443
ansible-playbook openshift-apply.yml -e kubeconfig_path=/secure/path/target.kubeconfig -e expected_api_server=https://api.cluster.example.com:6443
```
...
ansible-playbook kubernetes-module-apply.yml --check --diff -e kubeconfig_path=/secure/path/target.kubeconfig
```
...
- kubeadm ベースのセルフマネージド Kubernetes を自動構築する選択肢です。
...
Kubespray を採用する場合は、対応 Kubernetes／OS／Ansible、更新経路、コミュニティサポート、運用責任を評価します。
...
AAP を採用する場合は、対象版、execution environment、Credential、RBAC、バックアップ、更新方式を設計します。
...
- Inventory、変数、Role、template、handler を分離し、冪等性とレビュー可能性を保つ。
- 秘密情報は Vault や Automation Platform の Credential へ分離し、syntax、lint、check、diff、段階適用、post-check を実施する。
- OpenShift では Ansible と Operator/GitOps の所有範囲を決め、同じリソースを複数の仕組みで競合管理しない。
