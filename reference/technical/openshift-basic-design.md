...
この章は、OpenShift Container Platform（OCP）の基本設計で「何を合意し、何を後工程へ渡すか」を整理するための技術リファレンスです。個別製品の操作手順ではなく、要件から設計判断、試験、運用へつなぐ観点を扱います。
...
- 用途、可用性、障害ドメイン、運用境界、導入方式を先に決め、そこからノード、ネットワーク、ストレージへ設計を展開します。
...
- Control Plane のクォーラムと障害ドメインを守り、Worker の業務負荷と Ingress・監視などの基盤負荷を分けます。Infra Node 採用は規模、負荷、運用、製品サポート条件を確認して判断します。
...
- サイジングは平均 CPU 使用率だけではなく、requests、基盤 DaemonSet、障害・更新時の退避余力、Pod 数、ストレージ IOPS まで含めて算定します。
...
- API と `*.apps` の名前解決を利用元別に確認し、DNS、LB、証明書の FQDN を一致させます。具体的な必須レコードは導入方式と版の公式手順で確認します。
...
- API 系と Ingress 系の VIP、バックエンド、ヘルスチェック、TLS 終端、冗長性を分けて設計します。採用 LB 固有の設定とポートは要確認です。
...
- Route は OpenShift の外部公開リソースで、IngressController が実際の入口を担います。内部・外部の分離、TLS 終端、DNS/LB、配置と容量をまとめて設計します。
...
- Pod と Service の CIDR は将来規模と既存ネットワークとの非重複を確認して決めます。導入後の変更が難しいため、早い工程でネットワーク担当と合意します。
...
- 認証は既存 IdP 連携、MFA、グループ同期、緊急時アカウント、退職・異動時の失効まで設計対象です。連携方式と失効手順を非本番で検証します。
...
- Operator は導入可否だけでなく、提供元、Channel、承認方式、権限、CRD、更新試験、障害監視まで設計します。更新と障害時の責任分界も確認します。
...
- StorageClass は単なる保存先名ではなく、CSI、性能、アクセスモード、reclaimPolicy、暗号化、障害ドメイン、バックアップを抽象化する設計項目です。
...
- ODF の採用時は構成方式、障害ドメイン、デバイス、容量効率、再配置負荷、バックアップを確認します。
...
- 監視では収集対象だけでなく、閾値根拠、通知、一次対応、エスカレーション、保持容量、通知試験まで決めます。通知経路と一次対応を含めて試験します。
...
- ログは application、infrastructure、audit を分け、転送先、保持、権限、機密情報、転送障害時の動作を設計します。
...
- 運用設計では監視項目だけでなく、担当、判断基準、エスカレーション、変更証跡、更新、復元訓練まで具体化します。定期作業と例外時の判断も手順化します。
...
> 参照日は **2026-08-13** です。上記の版番号は本資料で用いる具体例であり、採用版を意味しません。実案件では対象 OCP の正確な z-stream、プラットフォーム、更新経路、サポートマトリクスを Red Hat 公式資料と契約条件で **要確認** です。

## 実務での説明要点

- 可用性とクラスタ境界から、ノード、DNS/LB、ネットワーク、認証・認可、Operator、ストレージへ設計を展開する。
- 監視、ログ、バックアップ、証明書、NTP、Firewall、運用、試験までを後付けにせず基本設計で合意する。
- 未決事項は推測で埋めず、担当、期限、影響、判断根拠を記録し、OCP 4.22 の公式資料と照合する。
