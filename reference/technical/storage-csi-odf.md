...
- PVC は要求、StorageClass はプロビジョニング方針、PV は割り当てられたストレージ、CSI はストレージ操作のインターフェースである。
- 容量だけでなく AccessMode、VolumeMode、Topology、性能、暗号化、Reclaim Policy、バックアップを設計する。
- OpenShift Virtualization の Live Migration、Snapshot、復元可否は OCP 4.22 と採用ストレージのサポート条件で確認する。
