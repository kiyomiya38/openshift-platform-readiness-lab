...
curl -sv --connect-timeout 5 -H 'X-Request-ID: example-request-001' https://<gateway-fqdn>/<api-path>
```
...
- Kong Gateway は API の入口で Route、認証・認可、Rate Limit、変換、ログなどを共通化する。
- AI Gateway では LLM provider のルーティング、トークン使用量、レイテンシ、コスト、ガードレールも管理対象になる。
- OpenShift では KIC、Route/LB、SCC/RBAC、NetworkPolicy、可用性、可観測性、Plugin と Edition の対応を設計する。
