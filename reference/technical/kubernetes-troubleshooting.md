...
- まず Pod の状態、`describe` の Events、現行・直前ログを採取し、スケジューリング前、コンテナ作成中、起動後のどこで失敗したかを特定します。
...
- 状態、Events、現行・直前ログを保全してから、Pod、Scheduler、Node、Network、Storage のどの層かを切り分ける。
- 影響範囲、発生時刻、直前変更、正常系との差を先に固定し、推測だけで再起動や削除を行わない。
- 対処には承認、影響評価、ロールバック、実施記録、復旧判定を伴わせる。
