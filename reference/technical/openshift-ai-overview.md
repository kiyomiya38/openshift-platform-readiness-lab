...
> 更新基準日: 2026-08-13。公式製品トップはOpenShift AI Self-Managed 3.5を表示するが、一部はEarly Accessであり、対応OCPのSupport MatrixはOCP本体と別Life Cycleである。本資料の具体例であるOCP 4.22と組み合わせ可能とは限らない。ComponentのGA/Technology Preview、対応OCP/GPU/Acceleratorを導入前に**要確認**。
...
- OpenShift AI は Workbench、Pipeline、Model Registry、Model Serving などを OpenShift 上で提供する AI/ML 基盤である。
- Dify などのアプリケーション層とは役割を分け、LLM、Embedding、RAG、Vector DB、認証、監査の境界を設計する。
- OCP 4.22 との互換性、GPU、ストレージ、モデルライセンス、GA/Technology Preview、Serving SLO を導入前に確認する。
