...
- API 用 DNS/LB、apps wildcard DNS と Ingress LB、Pod/Service Network、NetworkPolicy、Firewall 要件を分けて設計する。
- 不通時は DNS、TCP、TLS、HTTP、LB、Router、Service、EndpointSlice、Pod の順に境界を確認する。
- オンプレミスでは CIDR 重複、MTU、戻り経路、health check、Proxy/no_proxy を構築前に確定する。
