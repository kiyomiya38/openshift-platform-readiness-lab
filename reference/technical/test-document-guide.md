...
> [!NOTE]
> 本書は [基盤案件の進め方](infra-project-process.md) と空の試験テンプレートをつなぎ、実務の試験仕様・結果報告を読み解くためのガイドです。

記入済み成果物例として、架空案件の [試験仕様書](../../projects/enterprise-openshift-platform/docs/16-test-specification.md) と [試験結果報告書](../../projects/enterprise-openshift-platform/docs/17-test-results.md) を参照できます。全ケースの初期状態は `NOT RUN` であり、期待結果を実測結果として扱わないでください。

## 試験の目的

試験は操作の実施記録ではなく、「設計が受入条件を満たす」という証拠を残す活動です。[試験仕様書テンプレート](../../templates/test-spec-template.md)で事前に判定可能な期待結果を定め、[試験結果報告書テンプレート](../../templates/test-result-template.md)に実測値と証跡を残します。
...
- 要件・設計 ID から、前提、操作、期待結果、判定基準、証跡、後処理を試験仕様書へ展開する。
- 結果報告では期待値と実測値を分離し、Pass、Fail、Blocked、NOT RUN を曖昧にしない。
- 組織の試験標準と変更ルールに従い、差異、課題、再試験、残留リスク、完了判定を記録する。
