...
- Prometheus、Alertmanager、Grafana は状態把握と通知、Logging はログ収集・転送・保存・検索を担う。
- OADP は主にアプリケーション資源、etcd backup はコントロールプレーン復旧を扱うため、保護対象を混同しない。
- RPO/RTO、保持、権限、容量、通知試験、復元試験を設計し、バックアップ成功だけで完了としない。
