...
> [!IMPORTANT]
> 本書は架空の学習用入力を整理したもので、実環境調査の結果ではありません。実機構築・試験は未実施で、商用経験の証明ではありません。共通値は [SCENARIO.md](../SCENARIO.md) を正とします。
...
| CON-005 | 実検証環境がない | 構築・試験結果を生成できない | 静的検証と机上レビューに限定し未実施表示 | 学習者 |
