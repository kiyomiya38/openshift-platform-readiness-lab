...
> [!IMPORTANT]
> 本書は一般公開用の架空プロジェクトにおける基本設計サンプルです。状態はDraft・未レビュー・未承認で、実機構築、試験、製品互換性確認は未実施です。値は [SCENARIO.md](../SCENARIO.md) に基づく暫定値です。
...
| --- | --- | --- | --- | --- | --- |
| BD-001 | OCP 4.22.zを採用候補とする | 共通シナリオの具体例としてminor版を固定 | 他のサポート済み4.x | Platform | z版TBD |
...
1. 本番と同等条件の検証クラスタで対象更新を先行確認します（本サンプルには実環境がないためTBD）。
