...
| 文書版 | 0.1 Draft |
| 基準日 | 2026-08-17 |
| 性質 | 一般公開用の架空プロジェクト成果物サンプル |
| 実機構築 | 未実施 |
| 試験 | 未実施 |
| 技術レビュー／承認 | 未レビュー／未承認 |

本フォルダは、要件から設計、構築手順、試験、移行、運用引き継ぎまでで使用する成果物例を収録します。記載した値は架空の設計入力または暫定値であり、実環境へそのまま適用しません。秘密情報、実在する組織の情報、実IPアドレスは含めません。
...
サイジングは架空の入力値です。Red Hatの最小要件、ハードウェア互換性、VM収容計画、障害時退避余力、ストレージ性能を確認して確定します。

## 責任分界

各役割の意味、主な成果物、混同しやすい境界は[役割定義表](docs/03-scope-responsibility.md#role-definitions)を参照してください。
