...
この章では、Kubernetes を学習済みの人が OpenShift で追加・拡張される概念を、操作暗記ではなく「実務では何を確認するか」までつなげて整理します。

> **経験境界**
>
> - 商用 OpenShift の設計・構築・運用経験: **なし**
> - Developer Sandbox / CRC での Project、Route、Operator などの入門学習・教材作成: **本人申告に基づく教材・学習環境レベル**。RBAC、SCC、`oc` を含む具体的な操作範囲は提出前に本人確認します。
> - ClusterOperator、MachineConfig、BuildConfig、ImageStream、監視・ログ: **技術資料初稿・初回通読のみ（詳細レビュー前）**
> - 本章のコマンドは学習例であり、顧客環境での実行実績を意味しません。対象版と権限は **要確認** です。
...
「Kubernetes がコンテナオーケストレーションの中核で、OpenShift はそこへ認証、Route、SCC、Operator、更新、監視、ビルド・イメージ管理などを統合した製品だと理解しています。商用運用経験はありませんが、教材・検証環境で基本リソースを確認しています。」
...
「SCC は Pod の特権、UID、capability、ホスト資源利用を制限します。教材・検証レベルで確認しており、エラー時は privileged を安易に付けず、イベントと必要権限を比較します。」
...
「Operator は運用ロジックをコントローラー化したもので、OLM が CatalogSource、Subscription、InstallPlan、CSV を通して導入・更新を管理します。実導入経験はなく、教材・概要理解レベルです。」
...
「BuildConfig は OpenShift のビルド定義で、入力ソース、builder、出力イメージ、トリガーを管理します。商用 CI/CD 経験はなく、Pipelines や外部 CI との使い分けは概要理解レベルです。」
...
「OpenShift Monitoring は組み込みのプラットフォーム監視で、ユーザーワークロード監視とは責任を分けます。教材・概要理解レベルで、商用の保持容量や通知設計経験はありません。」
...
「OpenShift Logging はログ収集・転送・保存・検索の仕組みで、構成要素は版依存です。実導入経験はなく、application、infrastructure、audit の区分と保持・権限・転送障害の観点を整理したレベルです。」
...
「`oc` は `kubectl` の Kubernetes 操作を包含しつつ、Project、Route、Build、Image、`oc adm` など OpenShift 固有機能を追加した CLI と理解しています。」
...
「教材作成や検証中のエラー調査レベルです。Pod 障害では、範囲と変更時刻を確認し、`get`、`describe`、イベント、現在／直前ログを見て、Node、PVC、SCC、Secret、DNS・NetworkPolicy へ依存関係を広げます。」
...
> 参照日は **2026-08-13** です。リンク中の `4.22` は学習時点の参照例で、採用版を意味しません。API、既定値、非推奨機能、Logging の構成は変わり得るため、実案件では対象 z-stream の公式文書と `oc api-resources` で **要確認** です。

## 面談での説明例

「商用 OpenShift の構築・運用経験はありません。過去の経験は Developer Sandbox / CRC を用いた Project、Route、Operator などの入門学習・教材作成の範囲で、具体的な操作範囲は提出前に再確認します。この章には ClusterOperator、MachineConfig、BuildConfig、ImageStream、監視・ログを含む資料初稿があり、初回通読のみ完了しています。詳細レビューと演習はこれからです。」
