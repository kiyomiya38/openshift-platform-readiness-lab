...
> **経験境界**
>
> - ROSA: 商用設計・運用および本リポジトリでの実機検証は **経験なし**。過去の利用経験も確認できていません。
> - ARO: 商用利用・検証経験は **なし**。現在は資料初稿のみで、概要理解の有無も本人確認待ちです。
> - オンプレ OCP: 商用設計・構築・運用経験は **なし**
> - 機能、SLA、リージョン、料金、責任分界、導入方式、サポート条件は変わるため、契約時点で **要確認**
...
以下は read-only の学習例です。profile、region、cluster tag は実環境に合わせます。
...
「ROSA / ARO の商用経験と、本リポジトリでの実機検証はありません。現在は比較資料の初稿を一度通読し、これから詳細レビューを行う段階です。レビュー後は、ROSA が AWS、ARO が Azure 上の OpenShift サービスであること、責任分界、ネットワーク、ID、ストレージ、Load Balancer 等の確認観点を自分の言葉で説明できるか確認します。実施していない環境を利用経験としては説明しません。」
