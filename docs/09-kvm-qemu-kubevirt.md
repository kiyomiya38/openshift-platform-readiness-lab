...
> **経験境界**
>
> - 商用 KVM ホストの設計・構築・運用: **経験なし**
> - `virsh` を含む KVM/QEMU/libvirt の知識: **資料初稿・初回通読のみ（詳細レビュー前）**。過去の操作・検証経験は本人確認待ちです。
> - KubeVirt／OpenShift Virtualization: **構築経験なし**。現在は資料初稿のみで、概要理解の有無も本人確認待ちです。
> - CPU、デバイス、ストレージ、ネットワークの対応可否はハードウェアと製品版に依存するため **要確認** です。
...
「KubeVirt を知っている」ことは「OpenShift Virtualization の商用構築経験がある」ことを意味しません。製品では、対応 OCP 版、Operator の Channel、ノード要件、CSI/CNI、バックアップ、アップグレード、ゲスト OS サポートまで確認します。
...
ここでは実行手順を示すだけであり、実環境で VM を作成した経験の表明ではありません。
...
> 参照日は **2026-08-13** です。OCP `4.22` と RHEL 9 は学習時点の参照例です。デーモン名、コマンド、CPU／ゲスト OS／デバイス対応、nested virtualization、Live Migration は対象版とプラットフォームで **要確認** です。

## 面談での説明例

「商用環境で KVM ホストを設計・構築した経験はありません。KVM は Linux カーネルの仮想化機能、QEMU は VM と仮想デバイスを実行するユーザー空間コンポーネント、libvirt は従来型ホストでそれらを管理する API、KubeVirt は VM 管理を Kubernetes API に統合する仕組み、と概要を整理しています。OpenShift Virtualization では KVM を直接操作せず、VirtualMachine API、`oc`、`virtctl` を使うと理解しています。」
