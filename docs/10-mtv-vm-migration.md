...
> **経験境界**
>
> - 商用 VMware → OpenShift Virtualization 移行: **経験なし**
> - MTV の導入・実移行: **経験なし**。現在は資料初稿のみで、概要理解の有無も本人確認待ちです。
> - 本章の棚卸し、PoC、試験、切り戻し: **資料初稿・本人演習前**
> - 対応する移行元、ゲスト OS、cold/warm/live、VDDK、CSI/CNI、Technology Preview は MTV／OCP の版で変わるため **要確認** です。
...
- 認証情報、実 IP、ホスト名を公開教材や個人 AI へ投入しない。
...
資格情報は Secret として最小権限で管理し、Git、教材、チケット、AI へ平文を貼り付けません。vCenter 証明書検証を無効化する設定を恒常利用しないよう、信頼 CA を整備します。
...
> 参照日は **2026-08-13** です。MTV `2.10`、OCP `4.22` は学習時点の参照例です。cold/warm/live の対応元、必要版、Technology Preview、VDDK、ゲスト OS、ネットワーク／ストレージ制約は更新されるため、案件の組合せで **要確認** です。

## 面談での説明例

「MTV を使った商用 VM 移行経験はありません。概要理解レベルです。VMware から OpenShift Virtualization へ移す際は、VM 台数だけでなく、OS、CPU・メモリ実績、ディスク量と I/O、Port Group／VLAN、固定 IP、依存先、停止可能時間を棚卸しします。その上で StorageMap、NetworkMap、Plan を用いた PoC とリハーサルを行い、移行後の機能・性能・運用試験、バックアップ、データを含む切り戻し条件まで準備する必要があると理解しています。」
