...
> **経験境界**
>
> - 商用案件での OpenShift Virtualization 導入・構築・運用: **経験なし**
> - OpenShift Virtualization のアーキテクチャ、主要 API、設計論点: **資料初稿・初回通読のみ（詳細レビュー前）**。概要理解の有無も本人確認待ちです。
> - KVM/QEMU、KubeVirt、Multus、MTV: **資料初稿・初回通読のみ（詳細レビュー前）**。過去の検証経験は確認できていません。
> - 以下のコマンドや設定観点は学習用で、実環境で実施したという意味ではありません。ハードウェア、クラウド、ストレージ、ネットワーク、対象版のサポート条件は **要確認** です。
...
> 参照日は **2026-08-13** です。リンク中の OCP `4.22` は学習時点の参照例で、採用版を意味しません。OpenShift Virtualization、MTV、CSI、CNI の組合せ、Live Migration、対応ゲスト OS、Technology Preview の扱いは、対象 z-stream と Red Hat サポートマトリクスで **要確認** です。

## 面談での説明例

「OpenShift Virtualizationについては、実案件での導入・構築経験はありません。概要理解レベルです。

OpenShift上でコンテナだけでなく仮想マシンも管理・実行できる機能で、KubeVirt、KVM/QEMUをベースにしていると理解しています。

既存VMをすぐにコンテナ化できない場合でも、まずVMとしてOpenShift上に移行し、コンテナと同じ基盤で管理しながら、段階的なモダナイズにつなげられる点が価値だと考えています。

設計時には、VM 用ノードの退避余力、CPU 互換性、PVC／StorageClass、Live Migration、Multus と既存 VLAN、バックアップ、監視、MTV での移行後試験を確認します。実際の対応可否は対象版とサポートマトリクスで確認します。」
