...
> 更新基準日: 2026-08-13。法令、契約、顧客Rule、社内規程、AI Serviceの規約は変化する。本章は法的助言ではなく学習用であり、案件のSecurity/法務/契約担当へ**要確認**。
...
2026-08-13時点では経済産業省サイトにAI事業者ガイドライン第1.2版（2026-03-31）が掲載されている。将来改訂され得るため、面談・案件時点の最新版と社内規程を**要確認**。

## 面談での説明例

> AIは一般的なErrorや公開仕様の調査補助に使いますが、顧客情報、個人情報、実IP、Host名、Token、Log全文、構成図は入力しません。必要なら再識別できない形まで一般化し、顧客と社内のAI利用可否を先に確認します。得られた回答は採用Versionの公式Documentと検証環境で裏取りし、本番変更は手順書、Peer review、承認を通します。社内AIでも無条件に顧客Dataを入れてよいとは考えていません。
