...
> 経験境界: Airgap / Disconnected 環境での OpenShift 実構築経験はありません。`oc-mirror`、Mirror Registry、必要資材、設計観点は資料初稿・初回通読のみ（詳細レビュー前）で、概要理解の有無も未判定です。  
> 更新基準日: 2026-08-13。以下はOpenShift Container Platform 4.22系資料を学習例とする。`oc-mirror` v1/v2、ImageSetConfiguration API、Operator Catalog形式、対応Upgrade Pathは変化するため、案件採用Versionで**要確認**。
...
API VersionとChannelは4.22系の学習例である。`<mirror対象の4.22.z>`は、Supported Update Pathで確認した完全なz-streamへ置き換える。実在・対応・推奨Patchを実行前に**要確認**。`latest` は教材上の追加Image例にすぎず、本番資材はDigest固定を検討する。
...
> Airgap環境でのOpenShift構築経験はありません。
>
> Airgap、またはDisconnected環境は、クラスタがインターネットへ直接接続できない環境だと理解しています。
>
> そのため、OpenShiftのリリースイメージ、Operatorイメージ、必要なアプリケーションイメージを事前にMirror Registryへ同期し、クラスタは内部レジストリからイメージを取得する構成になります。
>
> 設計時には、Mirror Registry、oc-mirror、pull secret、証明書、DNS、Firewall許可、Operatorカタログ、アップデート用イメージ、NTP、Load Balancerなどを事前に整理する必要があると認識しています。
