...
> 経験境界: Sysdig の商用導入・運用・検証経験はありません。Sysdig Secure / Monitor、Falco、Agent、Kubernetes Audit の役割は資料初稿・初回通読のみ（詳細レビュー前）で、概要理解の有無も未判定です。  
...
- rule: Unexpected shell in container
  desc: Detect an interactive shell in a container for learning purposes
...
これは学習用であり、そのまま本番へ適用すると保守作業まで大量検知する。Macro/List/Exception、対象Workload、Expected behavior、Rule test、Ownerを設計する。利用可能FieldとSyntaxはFalco/Sysdig Versionで**要確認**。
...
> Sysdigの商用導入経験はありません。概要理解レベルです。Sysdig SecureはImage/Runtimeの脆弱性、Falcoを用いたRuntime threat、Kubernetes Audit等を扱うCloud-native Security製品と理解しています。OpenShiftへ導入する場合はAgentのSCC/RBAC、Kernel対応、負荷、SaaSへの通信とData residency、Rule tuning、SIEM連携を確認します。Prometheus/Grafanaは主にMetrics監視、RHACSは同じSecurity領域の別製品なので、要件と現行環境で比較します。
